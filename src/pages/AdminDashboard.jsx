...
export default function AdminDashboard() {
  ...
  const [modalType, setModalType] = useState(""); // EKSKUL, BERITA, USER
  ...
  const currentUserRole = localStorage.getItem("userRole"); // Cek Role yang login
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modalType === "EKSKUL") {
        const payload = { nama: formData.nama, kateg: formData.kateg, jadwal: formData.jadwal, desc: formData.desc, img: imageUrl };
        ...
        const payload = { judul: formData.judul, konten: formData.konten, tanggal: formData.tanggal, gambar: imageUrl };
        ...
        const payload = { 
          ...
          password: formData.password, // Plain text (simple)
      ...
      <Box bg="#0A2D5E" py={4} px={4} shadow="md" position="sticky" top="0" zIndex="999">
        <Container maxW="container.xl">
          <Flex justify="space-between" align="center">
            <Box>
              <Heading size="md" color="white" fontFamily="'Poppins', sans-serif">Dashboard Admin</Heading>
              <Text fontSize="xs" color="gray.300">Login sebagai: {localStorage.getItem("userName")}</Text>
            </Box>
            <Flex gap={3}>
              <Button leftIcon={<ExternalLinkIcon />} size="sm" variant="ghost" color="white" _hover={{ bg: "whiteAlpha.200" }} onClick={() => navigate("/")}>Lihat Web</Button>
      ...
      <Container maxW="container.xl" py={8}>
        <Tabs variant="enclosed" colorScheme="blue" isLazy>
          <TabList mb={6} overflowX="auto">
            ...
            {currentUserRole === 'admin' && <Tab fontWeight="bold">👥 Manajemen Akun</Tab>}
          </TabList>

          <TabPanels>
            <TabPanel p={0}>
              <Flex justify="end" mb={4}><Button leftIcon={<AddIcon />} bg="#FDC800" color="#0A2D5E" onClick={openAddEkskul}>Tambah Ekskul</Button></Flex>
              <Box bg="white" borderRadius="xl" shadow="sm" overflowX="auto">
                <Table variant="simple"><Thead bg="gray.50"><Tr><Th>Nama</Th><Th>Kategori</Th><Th>Aksi</Th></Tr></Thead>
                  <Tbody>{ekskuls.map((item) => (
                    <Tr key={item.id}><Td fontWeight="bold">{item.nama}</Td><Td><Badge>{item.kateg}</Badge></Td>
                      <Td>
            ...
            <TabPanel p={0}>
              <Flex justify="end" mb={4}><Button leftIcon={<AddIcon />} colorScheme="green" onClick={openAddBerita}>Buat Berita</Button></Flex>
              <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6}>
                {berita.map((item) => (
                  <Box key={item.id} bg="white" p={4} borderRadius="lg" shadow="sm">
                    ...
                    <Heading size="sm" mb={2}>{item.judul}</Heading>
                    <Flex gap={2}><Button size="xs" w="full" onClick={() => openEditBerita(item)}>Edit</Button><Button size="xs" colorScheme="red" onClick={() => handleDelete('berita', item.id)}>Hapus</Button></Flex>
                  </Box>
                ))}
              </SimpleGrid>
            </TabPanel>
            ...
            <TabPanel p={0}>
              <Box bg="white" borderRadius="xl" shadow="sm" overflowX="auto">
                <Table variant="simple" size="sm">
                  ...
                  <Tbody>{pendaftar.map((item) => (
                    <Tr key={item.id}><Td fontWeight="bold">{item.nama}</Td><Td>{item.kelas}</Td><Td>{item.ekskul}</Td>
            ...
            {currentUserRole === 'admin' && (
              <TabPanel p={0}>
                <Flex justify="space-between" mb={6}>
                  <Text color="gray.500">Kelola akun untuk Guru Pembina & Ketua Ekskul.</Text>
                  <Button leftIcon={<AddIcon />} colorScheme="purple" onClick={openAddUser}>Tambah User</Button>
                </Flex>
                <Box bg="white" borderRadius="xl" shadow="sm" overflowX="auto">
                  <Table variant="simple">
                    <Thead bg="gray.50"><Tr><Th>Nama Lengkap</Th><Th>Username</Th><Th>Role</Th><Th>Ekskul</Th><Th>Aksi</Th></Tr></Thead>
                    <Tbody>{users.map((item) => (
                      <Tr key={item.id}>
                        <Td fontWeight="bold">{item.nama_lengkap}</Td>
                        <Td>{item.username}</Td>
                        <Td><Badge colorScheme={item.role === 'admin' ? 'red' : item.role === 'pembina' ? 'blue' : 'green'}>{item.role}</Badge></Td>
                        <Td>{item.ekskuls?.nama || '-'}</Td>
                        <Td>
          ...
          <ModalBody pb={6}>
            ...
            {modalType === "EKSKUL" && (
              <>
                ...
                <FormControl mb={3}><FormLabel>Kategori</FormLabel><Select value={formData.kateg} onChange={(e)=>setFormData({...formData, kateg: e.target.value})}><option>Olahraga</option><option>Seni</option><option>Multimedia</option><option>Wajib</option></Select></FormControl>
                ...
                <FormControl mb={3}><FormLabel>Gambar</FormLabel><Input type="file" p={1} onChange={handleFileChange} /></FormControl>
              </>
            )}

            {modalType === "BERITA" && (
              <>
                <FormControl mb={3}><FormLabel>Judul</FormLabel><Input value={formData.judul} onChange={(e)=>setFormData({...formData, judul: e.target.value})} /></FormControl>
            ...
            {modalType === "USER" && (
              <>
                ...
                <FormControl mb={3}><FormLabel>Password</FormLabel><Input value={formData.password} onChange={(e)=>setFormData({...formData, password: e.target.value})} placeholder={isEditing ? "(Biarkan kosong jika tidak ubah)" :